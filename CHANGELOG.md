...
### Added
- `dilon-document-extractor`: heading-styled paragraphs that read like numbered procedure steps are now wrapped in a `@@@STEPS@@@`/`@@@END_STEPS@@@` block (nested by heading level, capped at 3 levels, closing/reopening around interruptions like images) instead of being flattened into nested list items

### Fixed
- `dilon-document-extractor`: table-to-markdown conversion now pads ragged rows to a common column count before computing column widths
